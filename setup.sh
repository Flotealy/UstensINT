...
# Configuration Backend FastAPI (avec préfixe USTENSINT_)
USTENSINT_DATABASE_URL=postgresql+asyncpg://ustensint:${POSTGRES_PASS}@db:5432/ustensint
USTENSINT_JWT_SECRET_KEY=${JWT_SECRET}
USTENSINT_JWT_ALGORITHM=HS256
USTENSINT_JWT_EXPIRATION_DAYS=7
USTENSINT_CORS_ORIGINS=["${PUBLIC_APP_URL}","http://localhost","http://127.0.0.1"]
USTENSINT_UPLOAD_DIR=/app/uploads
USTENSINT_MAX_UPLOAD_SIZE_MB=50
USTENSINT_ALLOWED_DOMAINS=telecom-sudparis.eu
EOF
